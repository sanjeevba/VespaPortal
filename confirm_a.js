...
$('<input type="button" name="ctl00$ContentContainer$WebFormControl_9a886c3c494aed11bba0001dd80507a7$NextButton" value="Confirm" onclick="updateStatusandType();javascript:if(typeof webFormClientValidate === \'function\'){if(webFormClientValidate()){if(typeof Page_ClientValidate === \'function\'){if(Page_ClientValidate(\'\')){clearIsDirty();disableButtons();this.value = \'Processing...\';}}else{clearIsDirty();disableButtons();this.value = \'Processing...\';}}else{return false;}}else{if(typeof Page_ClientValidate === \'function\'){if(Page_ClientValidate(\'\')){clearIsDirty();disableButtons();this.value = \'Processing...\';}}else{clearIsDirty();disableButtons();this.value = \'Processing...\';}};__doPostBack(\'ctl00$ContentContainer$WebFormControl_9a886c3c494aed11bba0001dd80507a7$NextButton\',\'\');navThankYou();" id="NextButton2" class="btn btn-primary button next submit-btn" nonactionlinkbutton="true">').insertAfter("#NextButton");
$("#NextButton").hide();
  _data = sessionStorage.getItem("grantAppData");
  data = JSON.parse(_data);
...
function updateStatusandType() {
 $('#ees_applicationstatus').val(276030000);
 $('#ees_granttype').val(1);
 var guid = sessionStorage.getItem('requestGuid');
 if (sessionStorage.getItem('TPGuidAppended') == 'false')
